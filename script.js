...
$(document).ready(function () {
    console.log("📌 Dokumentum betöltve, inicializálás...");

    let screenRefreshed = false; // Megakadályozza az ismételt frissítést

    function loadQuestionsFromDB(callback) {
        $.getJSON("/Question/GetQuestions", function (data) {
            console.log("✅ Kérdések sikeresen betöltve az adatbázisból:", data);
            questions = data;
            questionsLoaded = true;
            console.log("🔄 questionsLoaded állapota:", questionsLoaded);

            if (questions.length >= 27) {
                console.log("⏳ Már van 27 vagy több kérdés az adatbázisban, nem töltünk fel újakat.");
            ...
            // Csak egyszer frissítsen, ha még nem történt meg
    ...
    function refreshUI() {
        console.log("🔄 Képernyő frissítése egyszer...");
        setTimeout(() => {
            location.reload();
        }, 500);
    }
    ...
    $.getJSON("/Question/GetCategories", function (data) {
        console.log("✅ Kategóriák sikeresen betöltve:", data);
        categories = data;
    ...
    document.getElementById("help-category").addEventListener("change", function () {
        console.log("📌 Kategória változott:", this.value);
        showHelp(false);
    });

    document.getElementById("help-button").addEventListener("click", function () {
        console.log("📌 Segítség gombra kattintottak");
        showHelp(true);
    });
    ...
    loadQuestionsFromDB(() => {
        if (!questionsUploaded) {
            $.getJSON("questions.json", function (questions) {
                console.log("✅ questions.json sikeresen betöltve:", questions);

                function uploadQuestion(index) {
                    if (index >= questions.length) {
                        console.log("✅ Minden kérdés feltöltve.");
                        loadQuestionsFromDB();
                        return;
                    }

                    const question = questions[index];
                    console.log(`📌 (${index + 1}) Kérdés feltöltése:`, question);
                    ...
                    fetch("/Question/AddQuestion", {
                        ...
                        .then(response => {
                            console.log(`✅ (${index + 1}) Kérdés sikeresen feltöltve.`);
                ...
                console.log("HTTP státuszkód:", jqxhr.status);
                console.log("Válasz szöveg:", jqxhr.responseText);
            });
        } else {
            console.log("✅ Már van 27 vagy több kérdés, nem töltünk fel újakat.");
        }
    });

    generateQuestionsButton.addEventListener("click", () => {
        console.log("📌 'Kvíz generálása' gombra kattintottak.");
        console.log("🔄 questionsLoaded aktuális értéke:", questionsLoaded);
        ...
        if (!questionsLoaded) {
            ...
            loadQuestionsFromDB(() => {
                console.log("✅ Kvíz generálása elindul.");
                startQuiz();
            });
        } else {
            startQuiz();
...
function saveScore(username, score, totalPoints, percentage) {
    ...
    dataToSend.append("scoreValue", score); // Elért pontszám
    dataToSend.append("totalPoints", totalPoints); // Maximális pontszám
    dataToSend.append("percentage", percentage); // Százalékos eredmény
...
function listScore() {
    ...
    if (scoreContainer.children().length > 0) {
        scoreContainer.toggle(); // Ha már van tartalom, egyszerűen elrejtjük vagy megjelenítjük
    ...
    // Ha még nincs tartalom, betöltjük az adatokat és megjelenítjük
            ...
            response.scores.forEach(score => {
                ...
                scoresHtml += `
                    ...
                    ">
                        ...
                        <p><strong>Összszázalék</strong> 100%</p>
                        <p><strong>Időbélyeg:</strong> ${new Date(score.timestamp).toLocaleString()}</p>
                    </div>
                `;
            });

            scoreContainer.html(scoresHtml).show();
            scoreButton.text("Eredmények elrejtése"); // Gomb szöveg frissítése
        } else {
...
function showHelp(fromButton = false) {
    ...
    if (filteredQuestions.length === 0) {
        ...
        helpContent.innerHTML = ""; // 🔹 Törli az előző tartalmat
        ...
        filteredQuestions.forEach(item => {
            ...
            questionContainer.style.paddingBottom = "10px";
            ...
            answerElement.style.display = (category === "Táblák" || category === "Szituációk" || category === "Paragrafusok") ? "none" : "block";
            ...
            // Ha "Táblák", "Szituációk" vagy "Paragrafusok", akkor kattintásra mutatja/elrejti a választ
            if (category === "Táblák" || category === "Szituációk" || category === "Paragrafusok") {
